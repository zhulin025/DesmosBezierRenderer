...
* **✂️ 可确认的智能抠图**：上传后可从原图预览旁手动提取人物、建筑等主要主体；系统只先展示透明背景预览，用户确认后才会替换原图并重新绘制。若全部复杂度优化后仍达到 `5000` 条表达式上限，抠图预览会自动触发。
* **🛡️ 复杂图片保护**：自动限制处理像素、保边去噪并逐级调整缩放、噪点过滤和拟合容差，最多向 Desmos 注入 `5000` 条表达式。
* **⚡ 自动导入 Desmos**：图片上传、抠图确认或参数调整完成后，公式会直接更新到 Desmos，无需额外点击确认。
* **🎛️ 实时参数调优**：
...
2. 等待系统自动检测边缘并把公式导入 Desmos。
3. 如果主体不够突出，点击右下角原图卡片上的“智能抠图”。
4. 检查透明背景预览：选择“保留原图”，或点击“确认并重新绘制”。
5. 使用逐步绘制、PNG 下载或公式 TXT 下载；需要微调时再展开“算法参数调节”。
